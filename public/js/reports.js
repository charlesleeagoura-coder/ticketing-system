...
function buildPriorityChart(data) {
  ...
  const labels = data.map(d => d.priority.charAt(0).toUpperCase() + d.priority.slice(1));
  const values = data.map(d => d.count);
  const bgs     = data.map(d => (PRIORITY_COLORS[d.priority] || {bg:'#e2e8f0'}).bg);
  const borders = data.map(d => (PRIORITY_COLORS[d.priority] || {border:'#94a3b8'}).border);

  priorityChart = new Chart(ctx, {
    type: 'bar',
    data: { labels, datasets: [{ label: 'Tickets', data: values, backgroundColor: bgs, borderColor: borders, borderWidth: 2, borderRadius: 6 }] },
...
function buildTrendChart(yearData, year) {
  ...
  const labels  = yearData.map(m => MONTH_NAMES[m.month - 1].slice(0, 3));
  const opened  = yearData.map(m => m.opened);
  const closed  = yearData.map(m => m.closed);
  ...
  trendChart = new Chart(ctx, {
    ...
    data: {
      labels,
      datasets: [
        { label: 'Opened', data: opened, borderColor: '#3b82f6', backgroundColor: 'rgba(59,130,246,.12)', tension: 0.35, fill: true, pointRadius: 4, borderWidth: 2 },
        { label: 'Closed', data: closed, borderColor: '#10b981', backgroundColor: 'rgba(16,185,129,.12)', tension: 0.35, fill: true, pointRadius: 4, borderWidth: 2 }
      ]
    },
    options: {
...
function renderReport(report, yearlyData) {
  ...
  document.getElementById('rAvg').textContent =
    report.avg_resolution_hours != null ? report.avg_resolution_hours : '—';
  ...
  tbody.innerHTML = report.tickets.map(t => `
    <tr>
      <td><a href="/ticket/${t.id}" class="ticket-link">${escHtml(t.ticket_number)}</a></td>
      <td><a href="/ticket/${t.id}" style="color:var(--text);">${escHtml(t.title)}</a></td>
...
async function loadReport() {
  ...
  try {
    const [monthRes, yearRes] = await Promise.all([
      ...
      fetch(`/api/reports/yearly?year=${year}`)
    ]);
    const monthData = await monthRes.json();
    const yearData  = await yearRes.json();

    if (!monthData.success) throw new Error(monthData.error);
    if (!yearData.success)  throw new Error(yearData.error);

    renderReport(monthData.report, yearData.monthly);
  } catch (err) {
